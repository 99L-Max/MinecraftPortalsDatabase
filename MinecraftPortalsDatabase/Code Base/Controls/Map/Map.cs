...
namespace MinecraftPortalsDatabase
{
    partial class Map : UserControl
    {
        ...
        public Map(IEnumerable<MapPoint> points)
        {
            ...
            _squareHitRadius = MathCustom.GetSquare(points.Max(point => point.Size.Height));

            if (_points.Length > 1)
                _minScale = Screen.PrimaryScreen.Bounds.Size.Width / MathCustom.GetDifferenceMaxMin(_points) / 4f;
        ...
        {
            return new PointF((screen.X - _offset.X) / _scale, (screen.Y - _offset.Y) / _scale);
        }

        private RectangleF GetRectangleFOnScreen(MapPoint point)
        {
            return new RectangleF(point.Location.X * _scale + _offset.X, point.Location.Y * _scale + _offset.Y, point.Size.Width, point.Size.Height);
        }

        private string LocationToString(PointF point)
        {
            return $"{point.X:N0}; {point.Y:N0}";
        }
        ...
        private void Zoom(PointF point, int delta)
        {
            ...
            _scale = MathCustom.Clamp(newScale, _minScale, _maxScale);
        ...
        private void OnMouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                var rectangle = GetRectangleFOnScreen(point);
                var squareDistance = MathCustom.GetSquareDistance(rectangle.Location, e.Location);
